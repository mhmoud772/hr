...
// Arabic font support configuration
const configureArabicSupport = (doc: jsPDF) => {
  // For Arabic text, we'll use a workaround with reversed text direction
  doc.setR2L(true);
};
...
export const generateAttendanceReport = (
  ...
  doc.text("تقرير الحضور والانصراف", 105, 30, { align: "center" });
  
  doc.setFontSize(12);
  doc.text(`Date: ${reportDate}`, 105, 45, { align: "center" });
  ...
  doc.setFontSize(10);
  doc.text(`Total: ${records.length} | Present: ${present} | Absent: ${absent} | Late: ${late}`, 105, 55, { align: "center" });
  ...
  autoTable(doc, {
    ...
    body: records.map((record, index) => [
      ...
      record.checkOut,
      record.workHours,
      record.status === "حاضر" ? "Present" : record.status === "غائب" ? "Absent" : "Late"
    ]),
...
export const generateLeavesReport = (
  ...
  doc.text("تقرير طلبات الإجازات", 105, 30, { align: "center" });
